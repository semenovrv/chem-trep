const {QueryMol,SVG_MODE,BOOLEAN_MODE} = require('../build/Release/chem-trep')
	,os=require('os')
	,{EventEmitter}=require('events');

class CQuery extends QueryMol{
constructor(mol,mode){
	super(mol,...(mode?[SVG_MODE,mode.width,mode.height,false]:[BOOLEAN_MODE]))
}
//readMolfile(mol)
//test(mol)
...
#speed;
constructor(mol,mode){
	const ppp=mode?[SVG_MODE,mode.width,mode.height,false]:[BOOLEAN_MODE];
...
module.exports = {'MolSearch':MolSearch,'SDFStreamBase':SBase,'Query':CQuery,'SVG_MODE':SVG_MODE,'BOOLEAN_MODE':BOOLEAN_MODE};
